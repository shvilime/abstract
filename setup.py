...
class GradleDistribution(Distribution, object):
    PINNED_TXT = 'requirements.txt'

    excluded_platform_packages = {}

    def __init__(self, attrs):
        with open('gradle.properties') as file:
            metadata = dict(re.findall("__([a-z]+)__\s*=\s*([^\n]+)", file.read()))

            attrs['name'] = os.getenv('PYGRADLE_PROJECT_NAME') or metadata.get('name','UNKNOWN')
            attrs['version'] = os.getenv('PYGRADLE_PROJECT_VERSION') or metadata.get('version', 'UNKNOWN')
            attrs['author'] = metadata.get('author', '')
            attrs['author_email'] = metadata.get('email', '')
            attrs['description'] = metadata.get('description', '')
            attrs['url'] = metadata.get('url', '')
...
setup(
    ...
    include_package_data=True, install_requires=['requests']
)
